"""
app/services/workflow/workflow_executor.py

Workflow execution engine.

Architecture:

    Persisted Workflow
            |
            v
    WorkflowExecutor
            |
            +--> load Workflow
            |
            +--> load WorkflowSteps
            |
            +--> validate enabled steps
            |
            v
    WorkflowCompiler
            |
            v
       StageRegistry
            |
            v
    ProcessingPipeline
            |
            v
    pipeline.execute(context)
            |
            v
    ProcessingContext


Responsibilities
----------------

WorkflowExecutor is responsible for:

    - Loading persisted workflows
    - Loading persisted workflow steps
    - Validating workflow state
    - Filtering enabled steps
    - Preserving step order
    - Translating WorkflowStep models into compiler nodes
    - Delegating document-processing execution to WorkflowCompiler
    - Maintaining WorkflowExecutionContext
    - Maintaining a shared ProcessingContext
    - Tracking individual workflow-step state
    - Handling workflow execution failures

WorkflowCompiler is responsible for:

    - Resolving workflow operations
    - Resolving canonical ProcessingStage names
    - Resolving stages through StageRegistry
    - Building ProcessingPipeline instances

ProcessingPipeline is responsible for:

    - Executing ProcessingStage implementations

StageRegistry is responsible for:

    - Registering concrete ProcessingStage implementations

The WorkflowExecutor MUST NOT directly call:

    stage.process(...)

All document-processing execution must pass through:

    WorkflowCompiler
        ->
    ProcessingPipeline
        ->
    ProcessingStage
"""

from __future__ import annotations

import logging
from dataclasses import dataclass, field
from typing import Any, Mapping
from uuid import UUID, uuid4

from sqlalchemy.ext.asyncio import AsyncSession

from app.db.models.workflow import Workflow
from app.db.models.workflow_step import WorkflowStep
from app.processing.pipeline.context import ProcessingContext
from app.processing.stage_registry import StageRegistry
from app.processing.workflow.compiler import (
    CompiledWorkflow,
    WorkflowCompilationError,
    WorkflowCompiler,
)
from app.repositories.workflow.workflow_repository import (
    WorkflowRepository,
)
from app.repositories.workflow.workflow_step_repository import (
    WorkflowStepRepository,
)


logger = logging.getLogger(__name__)


# ============================================================================
# WORKFLOW EXECUTION CONTEXT
# ============================================================================


@dataclass
class WorkflowExecutionContext:
    """
    Runtime state for one workflow execution.

    This is the workflow-level context.

    Processing stages receive the shared ProcessingContext stored in:

        processing_context

    The same ProcessingContext is reused across processing steps so that:

        OCR
            |
            v
        extraction
            |
            v
        cleaning
            |
            v
        chunking
            |
            v
        embedding
            |
            v
        indexing

    can progressively enrich the same document state.
    """

    execution_id: UUID
    workflow_id: UUID
    input: dict[str, Any]

    output: Any = None

    steps: dict[str, dict[str, Any]] = field(
        default_factory=dict,
    )

    processing_context: ProcessingContext | None = None

    def current_input(self) -> Any:
        """
        Return the current workflow value.

        If a previous workflow-level step produced an output,
        that output becomes the current input.

        Otherwise the original workflow input is returned.
        """

        if self.output is not None:
            return self.output

        return self.input


# ============================================================================
# WORKFLOW EXECUTOR
# ============================================================================


class WorkflowExecutor:
    """
    Execute persisted Workflow definitions.

    The executor is the bridge between database workflows and
    the document-processing compiler.

    Architecture:

        Workflow
            |
            v
        WorkflowExecutor
            |
            +--> WorkflowRepository
            |
            +--> WorkflowStepRepository
            |
            v
        WorkflowCompiler
            |
            v
        StageRegistry
            |
            v
        ProcessingPipeline
            |
            v
        ProcessingContext
            |
            v
        ProcessingStage

    The executor does not implement document-processing logic.
    """

    def __init__(
        self,
        db: AsyncSession,
        stage_registry: StageRegistry,
    ) -> None:
        """
        Initialize the workflow executor.

        Parameters
        ----------
        db:
            SQLAlchemy async database session.

        stage_registry:
            Application-wide StageRegistry containing registered
            document-processing stages.
        """

        self.db = db

        self.workflow_repository = WorkflowRepository(
            db,
        )

        self.workflow_step_repository = WorkflowStepRepository(
            db,
        )

        self.stage_registry = stage_registry

        self.compiler = WorkflowCompiler(
            stage_registry,
        )

    # ========================================================================
    # PUBLIC API
    # ========================================================================

    async def execute(
        self,
        workflow_id: UUID,
        *,
        input_data: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        """
        Execute a persisted workflow.

        This is the primary public execution API.
        """

        return await self.start_execution(
            workflow_id,
            input_data=input_data,
        )

    async def start_execution(
        self,
        workflow_id: UUID,
        *,
        input_data: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        """
        Start execution of a persisted workflow.

        Execution flow:

            1. Load Workflow
            2. Validate workflow
            3. Load WorkflowSteps
            4. Sort by step_order
            5. Filter disabled steps
            6. Create WorkflowExecutionContext
            7. Execute enabled steps
            8. Return execution result
        """

        # --------------------------------------------------------------------
        # Load workflow
        # --------------------------------------------------------------------

        workflow = await self.workflow_repository.get_by_id(
            workflow_id,
        )

        if workflow is None:
            raise ValueError(
                f"Workflow '{workflow_id}' not found.",
            )

        if not workflow.is_active:
            raise ValueError(
                f"Workflow '{workflow_id}' is not active.",
            )

        # --------------------------------------------------------------------
        # Load workflow steps
        # --------------------------------------------------------------------

        steps = await self.workflow_step_repository.list_by_workflow(
            workflow_id,
        )

        steps = self._sort_steps(
            steps,
        )

        enabled_steps = [
            step
            for step in steps
            if step.is_enabled
        ]

        if not enabled_steps:
            raise ValueError(
                f"Workflow '{workflow.name}' contains no enabled steps.",
            )

        # --------------------------------------------------------------------
        # Create execution context
        # --------------------------------------------------------------------

        execution_id = uuid4()

        context = WorkflowExecutionContext(
            execution_id=execution_id,
            workflow_id=workflow_id,
            input=input_data or {},
        )

        logger.info(
            "Starting workflow execution.",
            extra={
                "execution_id": str(execution_id),
                "workflow_id": str(workflow_id),
                "workflow_name": workflow.name,
                "step_count": len(enabled_steps),
            },
        )

        # --------------------------------------------------------------------
        # Execute workflow steps
        # --------------------------------------------------------------------

        try:
            for step in enabled_steps:
                await self._run_step(
                    step=step,
                    context=context,
                )

        except Exception:
            logger.exception(
                "Workflow execution failed.",
                extra={
                    "execution_id": str(execution_id),
                    "workflow_id": str(workflow_id),
                    "workflow_name": workflow.name,
                },
            )

            raise

        # --------------------------------------------------------------------
        # Complete
        # --------------------------------------------------------------------

        logger.info(
            "Workflow execution completed.",
            extra={
                "execution_id": str(execution_id),
                "workflow_id": str(workflow_id),
                "workflow_name": workflow.name,
            },
        )

        return self._build_execution_result(
            workflow=workflow,
            context=context,
        )

    # ========================================================================
    # STEP EXECUTION
    # ========================================================================

    async def _run_step(
        self,
        *,
        step: WorkflowStep,
        context: WorkflowExecutionContext,
    ) -> None:
        """
        Execute one enabled WorkflowStep.

        WorkflowStep state is tracked inside WorkflowExecutionContext.
        """

        step_id = str(step.id)

        step_state = {
            "status": "running",
            "step_id": step_id,
            "name": step.name,
            "step_type": step.step_type,
            "step_order": step.step_order,
        }

        context.steps[step_id] = step_state

        logger.info(
            "Executing workflow step.",
            extra={
                "execution_id": str(context.execution_id),
                "workflow_id": str(context.workflow_id),
                "step_id": step_id,
                "step_name": step.name,
                "step_type": step.step_type,
                "step_order": step.step_order,
            },
        )

        try:
            result = await self.execute_step(
                step=step,
                context=context,
            )

            context.steps[step_id] = {
                **step_state,
                "status": "completed",
                "result": result,
            }

            context.output = result

        except Exception as exc:
            context.steps[step_id] = {
                **step_state,
                "status": "failed",
                "error": str(exc),
            }

            logger.exception(
                "Workflow step failed.",
                extra={
                    "execution_id": str(context.execution_id),
                    "workflow_id": str(context.workflow_id),
                    "step_id": step_id,
                    "step_name": step.name,
                },
            )

            # Current execution strategy is fail-fast.
            raise

    async def execute_step(
        self,
        *,
        step: WorkflowStep,
        context: WorkflowExecutionContext,
    ) -> Any:
        """
        Execute one WorkflowStep according to its step_type.

        Supported workflow-level step types:

            input
            processing
            ai
            knowledge
        """

        step_type = self._normalize_step_type(
            step.step_type,
        )

        if step_type == "input":
            return await self._execute_input_step(
                step=step,
                context=context,
            )

        if step_type == "processing":
            return await self._execute_processing_step(
                step=step,
                context=context,
            )

        if step_type == "ai":
            return await self._execute_ai_step(
                step=step,
                context=context,
            )

        if step_type == "knowledge":
            return await self._execute_knowledge_step(
                step=step,
                context=context,
            )

        raise ValueError(
            f"Unsupported workflow step type "
            f"'{step.step_type}' for step '{step.name}'.",
        )

    # ========================================================================
    # INPUT
    # ========================================================================

    async def _execute_input_step(
        self,
        *,
        step: WorkflowStep,
        context: WorkflowExecutionContext,
    ) -> dict[str, Any]:
        """
        Execute an input step.

        Input steps expose the original workflow input.
        """

        logger.debug(
            "Executing input workflow step.",
            extra={
                "execution_id": str(context.execution_id),
                "step_id": str(step.id),
            },
        )

        return dict(
            context.input,
        )

    # ========================================================================
    # PROCESSING
    # ========================================================================

    async def _execute_processing_step(
        self,
        *,
        step: WorkflowStep,
        context: WorkflowExecutionContext,
    ) -> Any:
        """
        Execute a document-processing workflow step.

        IMPORTANT:

        This method does NOT directly call:

            stage.process(...)

        Instead:

            WorkflowStep
                |
                v
            compiler node
                |
                v
            WorkflowCompiler
                |
                v
            StageRegistry
                |
                v
            ProcessingPipeline
                |
                v
            pipeline.execute(context)

        The shared ProcessingContext is reused between processing steps.
        """

        processing_context = (
            self._get_or_create_processing_context(
                context=context,
            )
        )

        # --------------------------------------------------------------------
        # Convert persisted WorkflowStep -> compiler node
        # --------------------------------------------------------------------

        node = self._workflow_step_to_compiler_node(
            step,
        )

        logger.info(
            "Compiling processing workflow step.",
            extra={
                "execution_id": str(context.execution_id),
                "workflow_id": str(context.workflow_id),
                "step_id": str(step.id),
                "step_name": step.name,
                "operation": node["operation"],
            },
        )

        # --------------------------------------------------------------------
        # Compile
        # --------------------------------------------------------------------

        try:
            compiled = self.compiler.compile_nodes(
                [node],
                fail_fast=True,
            )

        except WorkflowCompilationError:
            logger.exception(
                "Failed to compile workflow processing step.",
                extra={
                    "execution_id": str(context.execution_id),
                    "workflow_id": str(context.workflow_id),
                    "step_id": str(step.id),
                    "step_name": step.name,
                },
            )

            raise

        # --------------------------------------------------------------------
        # Execute pipeline
        # --------------------------------------------------------------------

        result = await self._execute_compiled_pipeline(
            compiled=compiled,
            context=processing_context,
        )

        # --------------------------------------------------------------------
        # Return workflow-friendly result
        # --------------------------------------------------------------------

        return self._build_processing_result(
            compiled=compiled,
            pipeline_result=result,
            processing_context=processing_context,
        )

    async def _execute_compiled_pipeline(
        self,
        *,
        compiled: CompiledWorkflow,
        context: ProcessingContext,
    ) -> Any:
        """
        Execute a compiled ProcessingPipeline.

        The WorkflowExecutor delegates actual processing to the
        ProcessingPipeline.
        """

        logger.info(
            "Executing compiled processing pipeline.",
            extra={
                "stage_names": compiled.stage_names,
                "node_ids": compiled.node_ids,
            },
        )

        return await compiled.pipeline.execute(
            context,
        )

    # ========================================================================
    # WORKFLOW STEP -> COMPILER NODE
    # ========================================================================

    @staticmethod
    def _workflow_step_to_compiler_node(
        step: WorkflowStep,
    ) -> dict[str, Any]:
        """
        Convert a persisted WorkflowStep into the node format expected
        by WorkflowCompiler.

        Persisted database model:

            WorkflowStep(
                id=...,
                name="OCR",
                step_type="processing",
                config={
                    "stage": "ocr"
                }
            )

        becomes:

            {
                "id": "...",
                "operation": "ocr",
                "config": {...},
            }

        Supported configuration:

            {
                "stage": "ocr"
            }

        or:

            {
                "stage_name": "ocr"
            }

        or:

            {
                "operation": "ocr"
            }

        The explicit configuration takes precedence over the
        WorkflowStep name.
        """

        config = (
            dict(step.config)
            if isinstance(step.config, Mapping)
            else {}
        )

        operation = (
            config.get("operation")
            or config.get("stage")
            or config.get("stage_name")
        )

        # --------------------------------------------------------------------
        # Fallback to step name
        # --------------------------------------------------------------------

        if operation is None:
            operation = step.name

        if not isinstance(
            operation,
            str,
        ):
            raise ValueError(
                f"Invalid processing operation for workflow step "
                f"'{step.name}'. Expected a string.",
            )

        operation = operation.strip()

        if not operation:
            raise ValueError(
                f"Workflow step '{step.name}' does not define "
                "a processing operation.",
            )

        return {
            "id": str(step.id),
            "operation": operation,
            "config": config,
            "data_mapping": (
                dict(step.data_mapping)
                if isinstance(
                    step.data_mapping,
                    Mapping,
                )
                else {}
            ),
        }

    # ========================================================================
    # PROCESSING CONTEXT
    # ========================================================================

    def _get_or_create_processing_context(
        self,
        *,
        context: WorkflowExecutionContext,
    ) -> ProcessingContext:
        """
        Get or create the shared ProcessingContext.

        The first processing step creates the context.

        Every subsequent processing step reuses the same context.
        """

        if context.processing_context is not None:
            return context.processing_context

        input_data = context.input

        document_id = input_data.get(
            "document_id",
        )

        if document_id is None:
            raise ValueError(
                "Processing workflow requires "
                "'document_id' in workflow input.",
            )

        processing_context = ProcessingContext(
            document_id=str(
                document_id,
            ),
            file_path=self._optional_string(
                input_data.get("file_path"),
            ),
            file_name=self._optional_string(
                input_data.get("file_name"),
            ),
            mime_type=self._optional_string(
                input_data.get("mime_type"),
            ),
            file_size=self._optional_int(
                input_data.get("file_size"),
            ),
            checksum=self._optional_string(
                input_data.get("checksum"),
            ),
            raw_text=self._optional_string(
                input_data.get("raw_text"),
            ),
            metadata=self._dictionary(
                input_data.get("metadata"),
            ),
        )

        context.processing_context = processing_context

        logger.debug(
            "Created shared ProcessingContext.",
            extra={
                "execution_id": str(context.execution_id),
                "workflow_id": str(context.workflow_id),
                "document_id": str(document_id),
            },
        )

        return processing_context

    # ========================================================================
    # PROCESSING RESULT
    # ========================================================================

    @staticmethod
    def _build_processing_result(
        *,
        compiled: CompiledWorkflow,
        pipeline_result: Any,
        processing_context: ProcessingContext,
    ) -> dict[str, Any]:
        """
        Convert ProcessingPipeline output into workflow output.
        """

        return {
            "status": (
                "failed"
                if processing_context.is_failed
                else "completed"
            ),
            "stages": list(
                compiled.stage_names,
            ),
            "node_ids": list(
                compiled.node_ids,
            ),
            "result": pipeline_result,
            "document": {
                "document_id": (
                    processing_context.document_id
                ),
                "file_name": (
                    processing_context.file_name
                ),
                "mime_type": (
                    processing_context.mime_type
                ),
            },
            "classification": {
                "document_type": (
                    processing_context.document_type
                ),
                "document_subtype": (
                    processing_context.document_subtype
                ),
                "confidence": (
                    processing_context.classification_confidence
                ),
            },
            "content": {
                "raw_text": (
                    processing_context.raw_text
                ),
                "cleaned_text": (
                    processing_context.cleaned_text
                ),
            },
            "metadata": (
                processing_context.metadata
            ),
            "extracted_data": (
                processing_context.extracted_data
            ),
            "completed_stages": (
                processing_context.completed_stages
            ),
            "failed_stages": (
                processing_context.failed_stages
            ),
            "warnings": (
                processing_context.warnings
            ),
            "errors": (
                processing_context.errors
            ),
        }

    # ========================================================================
    # AI
    # ========================================================================

    async def _execute_ai_step(
        self,
        *,
        step: WorkflowStep,
        context: WorkflowExecutionContext,
    ) -> dict[str, Any]:
        """
        Execute an AI workflow step.

        The AI service/agent layer can be connected here later.

        This method intentionally does not pretend that an AI stage is
        a document-processing ProcessingStage.
        """

        current_input = context.current_input()

        logger.debug(
            "Executing AI workflow step.",
            extra={
                "execution_id": str(context.execution_id),
                "workflow_id": str(context.workflow_id),
                "step_id": str(step.id),
                "step_name": step.name,
            },
        )

        return {
            "status": "completed",
            "step": step.name,
            "step_type": step.step_type,
            "input": current_input,
            "config": step.config or {},
        }

    # ========================================================================
    # KNOWLEDGE
    # ========================================================================

    async def _execute_knowledge_step(
        self,
        *,
        step: WorkflowStep,
        context: WorkflowExecutionContext,
    ) -> dict[str, Any]:
        """
        Execute a knowledge/retrieval workflow step.

        The knowledge/retrieval service can be connected here later.
        """

        current_input = context.current_input()

        logger.debug(
            "Executing knowledge workflow step.",
            extra={
                "execution_id": str(context.execution_id),
                "workflow_id": str(context.workflow_id),
                "step_id": str(step.id),
                "step_name": step.name,
            },
        )

        return {
            "status": "completed",
            "step": step.name,
            "step_type": step.step_type,
            "input": current_input,
            "config": step.config or {},
        }

    # ========================================================================
    # STEP SORTING
    # ========================================================================

    @staticmethod
    def _sort_steps(
        steps: list[WorkflowStep],
    ) -> list[WorkflowStep]:
        """
        Sort persisted workflow steps by WorkflowStep.step_order.

        IMPORTANT:

        The database model uses:

            step_order

        not:

            position
        """

        return sorted(
            steps,
            key=lambda step: (
                step.step_order,
                str(step.id),
            ),
        )

    # ========================================================================
    # STEP TYPE NORMALIZATION
    # ========================================================================

    @staticmethod
    def _normalize_step_type(
        step_type: str,
    ) -> str:
        """
        Normalize workflow step type.

        Examples:

            "AI"
                -> "ai"

            " Processing "
                -> "processing"
        """

        if not isinstance(
            step_type,
            str,
        ):
            raise ValueError(
                "Workflow step type must be a string.",
            )

        normalized = step_type.strip().lower()

        if not normalized:
            raise ValueError(
                "Workflow step type cannot be empty.",
            )

        return normalized

    # ========================================================================
    # VALUE HELPERS
    # ========================================================================

    @staticmethod
    def _optional_string(
        value: Any,
    ) -> str | None:
        """
        Safely convert an optional value to string.
        """

        if value is None:
            return None

        if isinstance(
            value,
            str,
        ):
            return value

        return str(value)

    @staticmethod
    def _optional_int(
        value: Any,
    ) -> int | None:
        """
        Safely convert an optional value to int.
        """

        if value is None:
            return None

        try:
            return int(value)

        except (
            TypeError,
            ValueError,
        ) as exc:
            raise ValueError(
                f"Invalid integer value: {value!r}",
            ) from exc

    @staticmethod
    def _dictionary(
        value: Any,
    ) -> dict[str, Any]:
        """
        Safely normalize a dictionary value.
        """

        if value is None:
            return {}

        if not isinstance(
            value,
            Mapping,
        ):
            raise ValueError(
                "Expected a dictionary value.",
            )

        return dict(value)

    # ========================================================================
    # PUBLIC RESULT
    # ========================================================================

    @staticmethod
    def _build_execution_result(
        *,
        workflow: Workflow,
        context: WorkflowExecutionContext,
    ) -> dict[str, Any]:
        """
        Build the public workflow execution result.
        """

        processing_context = (
            context.processing_context
        )

        result: dict[str, Any] = {
            "execution_id": context.execution_id,
            "workflow_id": context.workflow_id,
            "workflow_name": workflow.name,
            "workflow_version": workflow.version,
            "status": "completed",
            "input": context.input,
            "output": context.output,
            "steps": context.steps,
        }

        # --------------------------------------------------------------------
        # Processing information
        # --------------------------------------------------------------------

        if processing_context is not None:
            result["processing"] = {
                "document_id": (
                    processing_context.document_id
                ),
                "current_stage": (
                    processing_context.current_stage
                ),
                "completed_stages": (
                    processing_context.completed_stages
                ),
                "failed_stages": (
                    processing_context.failed_stages
                ),
                "document_type": (
                    processing_context.document_type
                ),
                "document_subtype": (
                    processing_context.document_subtype
                ),
                "classification_confidence": (
                    processing_context.classification_confidence
                ),
                "raw_text": (
                    processing_context.raw_text
                ),
                "cleaned_text": (
                    processing_context.cleaned_text
                ),
                "metadata": (
                    processing_context.metadata
                ),
                "extracted_data": (
                    processing_context.extracted_data
                ),
                "stage_results": (
                    processing_context.stage_results
                ),
                "errors": (
                    processing_context.errors
                ),
                "warnings": (
                    processing_context.warnings
                ),
                "duration_seconds": (
                    processing_context.duration_seconds
                ),
            }

        return result


__all__ = [
    "WorkflowExecutionContext",
    "WorkflowExecutor",
]